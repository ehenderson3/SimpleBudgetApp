...
class BudgetData: ObservableObject {
    ...
    @Published var savingsBuckets: [SavingsBucket] = [] {
        didSet {
            saveSavingsBuckets()
        }
    }
    ...
    func saveSavingsBuckets() {
        let encoder = JSONEncoder()
        do {
            let encodedBuckets = try encoder.encode(savingsBuckets)
            UserDefaults.standard.set(encodedBuckets, forKey: "savingsBuckets")
        } catch {
            print("Failed to encode savings buckets: \(error.localizedDescription)")
        }
    }
    
    func loadSavingsBuckets() {
        let decoder = JSONDecoder()
        do {
            if let savedBucketsData = UserDefaults.standard.data(forKey: "savingsBuckets") {
                savingsBuckets = try decoder.decode([SavingsBucket].self, from: savedBucketsData)
            }
        } catch {
            print("Failed to decode savings buckets: \(error.localizedDescription)")
        }
    }
    ...
    func updateExpense(expenseID: UUID, name: String, amount: Double, categoryID: UUID) {
